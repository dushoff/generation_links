...
% \jd{This is our way of formulating the Euler-Lotka equation. Why should we re-credit WallLips07?}
...
\jd{It is not clear here where we switch from talking about our approximation to talking about the empirical version.}
...
\jd{If this is necessary, it should be in rabies section:} For simplicity, we assumed that latent and infectious periods are equivalent to incubation and symptomatic periods for EVD and canine rabies.
...
To test whether gamma moment matching works for high \RR\ values, we applied the moment approximation to outbreaks of measles in Britain during year 1956 and 1970 \jd{What does this mean? Can we instead say where we get g-i and other information?:} assuming that they all share same generation interval distributions. Here, we found that the approximation matches the theoretical distribution almost perfectly (no visible differences in the curves) across our range of interest (\RR\ up to $>20$).
