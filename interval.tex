...
A few studies have explored how changes in certain features of the generation interval distributions translates to changes in the \rR\ relationship.
\cite{WallLips07} explored the \rR\ relationship under various distributional assumptions (e.g., exponential, delta, and normal) and explored how their underlying parameters affect the \rR\ relationsip.
On the other hand, \cite{wearing2005appropriate, roberts2007model} mainly focused on \rR\ relationship derived from compartmental models.
They studied consequences of changes in incubation and infectious period distributions and difference between \RR\ estimates under different assumptions.
However, \cite{wearing2005appropriate, roberts2007model} presented a seemingly contradictory result that more variable infectious period distribution leads to opposite effect in \RR.
...
The primary goal of this paper is to provide intuitive understanding behind the \rR\ relationship for wider audience.
...
We give simple, biological explanations for previous findings by relating them to means and variance measures of generation interval distributions.
...
\swp{I think I did this last week.}
\jd{Do we think that the effect on mean is always stronger? Why or why not?}
\swp{No. I think there's something about constant contact rate. If we relax that assumption, we might see a difference. For example, if infections are concentrated during early phase (due to control measures such as hospitalization or characteristic of a disease such as HIV), it doesn't matter when they recover and effect on mean might be small. On the other hand, if infections are concentrated near the end of infectious periods, effect on mean will be large.}
