...
\jd{Brief summary of main things done by these three papers. Don't stress much about what's missing from their work at this stage. Do mention the thing that's apparently contradictory.}
...
shorter mean latent and infectious periods are expected to result in smaller \RR\ and less-variable latent periods are expected to result in higher \RR.
Less-variable infectious periods have However, as decreasing coefficient of variation of infectious period leads to shorter mean generation interval as well \cite{Sven07}, assuming a tighter infectious period distribution does not necessarily result in higher \RR \cite{wearing2005appropriate}.
}
...
\jd{Daniel: Find all of the notes we ever did together! LMK if there's anything about this.}
\jd{Do we think that the effect on mean is always stronger? Why or why not?}
...
We also provided mechanistic interpretations: when generation intervals are longer, more infection is needed per generation (larger \RR)  in order to produce a given rate of increase $r$. Similarly, when variance in generation time is low, there is less early infection, and thus slower exponential growth for a given value of \RR; thus a higher value of \RR\ will be needed to match a given value of $r$. \jd{Let's think a bit about this recent clarification attempt.}
