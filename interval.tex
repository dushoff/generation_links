...
\begin{figure}[htbp] \centering
	...
	\caption{Two hypothetical epidemics with the same growth rate ($r=XXX$).  Assuming a short generation interval (fast transmission at the individual level) implies a smaller value for the reproduction number $\Ro$ (panel A) when compared to a longer generation interval (slow transmission at the individual level, panel B). \jd{Show \RR\ values on graphs.}
	\label{fig:link}}
\end{figure}
