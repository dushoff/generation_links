...
Generation interval distributions can be difficult to ascertain empirically \dc{Cite Champredon,Dushoff RSPB 2015: where we show the difficulty of such estimation} \jd{Our paper is somewhat relevant, but it feels weird to cite just that.}, and the generation-function approach depends on an entire distribution -- which makes it difficult to determine which features of the distributions are essential to connect measurements of the rate of spread $r$, with the reproductive number, \RR.
