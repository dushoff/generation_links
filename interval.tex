...
\begin{equation}
\tsup{R}}{gamma} &= 1 + \rho + \frac{(1-\kappa)}{2} \rho^2 + O(\rho^3)
\end{equation}
...
\subsection{SEIR differential equations model}
\label{sub:seminr}

Many epidemics can be characterized by SEIR models... [add text later; cite WL, Roberts, Wearing].
For this model, \rR\ relationship is given by
\begin{equation}
...
A potential concern for the gamma approximation is that it predicts a convex \rR\ relationship and there may exist generation intervals distribution that predicts a concave \rR\ relationship.
...
\begin{equation}
2 \left(\int{\tau g(\tau) \exp(-r\tau) \,d\tau}\right)^2 > \left(\int{g(\tau)\exp(-r \tau) \,d\tau}\right)\left( \int{\tau^2 g(\tau) \exp(-r\tau) \,d\tau}\right)
\end{equation}
