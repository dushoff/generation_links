...
{
\Large
\textbf\newline{Inferring the strength of epidemics from their speed: a feasible generation interval-based approach}
}
\newline
\\
...
We explore approximations based on estimates of the mean and variance of an underlying gamma distribution, and find that use of these two moments -- which can be feasibly measured in practice -- can be sufficient to capture the \rR\ relationship and provide robust estimates of \RR\ while an outbreak is ongoing.
...
\jsw{A title with clarifying won't end up in a top journal}
...
Generation interval distributions can be difficult to ascertain empirically \dc{Cite Champredon,Dushoff RSPB 2015: where we show the difficulty of such estimation} \jd{Our paper is somewhat relevant, but it feels weird to cite just that.} \swp{Let's cite PRSB 2015 and Nishiura.}, and the generation-function approach depends on an entire distribution.
...
Moreover, making different assumptions about the shape of generation interval distributions have lead to seemingly contradictory results \cite{wearing2005appropriate, roberts2007model}.
\swp{Incorporated JSW's changes and made minor edits. I agree with removing the paragraph that came after this but we should perhaps foreshadow that different assumptions lead to seemingly contradictory results but can be explained under our framework?}
...
\hline
Normal approximation & \tabitem Mean/SD generation interval & \tabitem Assumes normally distributed generation intervals & \cite{wallinga2007generation, trichereau2012estimation}\\
...
\swp{Unifying previous results and providing a feasible method?}
...
We show that summarizing generation-interval distributions with its moments provide biological explanations that unify previous findings.
...
While the gamma approximation has been applied to infer \RR\ in real outbreak settings previosuly (\tref{review}), its validity and biological implication has not been investigated yet.
\swp{Moved the last sentence from discussion to here.}
...
We write:
\begin{equation}
 	 \tsup{\RR}{gamma} \equiv (1 + \kappa \rho)^{1/\kappa},
	\label{eq:genExp}
\end{equation}
...
We then explore the behaviour of the gamma-approximated speed-strength relationship $\tsup{\RR}{gamma}$ defined above (equivalent to the Tsallis ``q-exponential'', with $q=1-\kappa$ \cite{tsallis1994numbers}): its shape determines how the estimate of $\RR$ changes with the estimate of normalized generation length $\rho$.
...
For small $\rho$, $\tsup{\RR}{gamma}$ always looks like $1+\rho$, regardless of the shape parameter $1/\kappa$, which determines the curvature: if $1/\kappa = 1$, we get a straight line, for $1/\kappa=2$ the curve is quadratic, and so on (see \fref{genExp}).
For large values of $1/\kappa$, $\tsup{\RR}{gamma}$ converges toward $\exp(\rho)$.

\dc{I think we could do without the $X$ function. It's not so well known that it provides help for the reader... I'd even say it's slightly annoying. It's just used here and doesn't bring much. Just talk in terms of \R, I think it will be clearer, straight to the point. }
\jd{Not sure how to talk about it without a name. We can't quite call it \RR, because $X$ is an approximation of that. We could call it \RRhat (if we can figure out how to put the hat on top), but I'm not sure why that's better.}
\swp{How about $\tsup{\RR}{gamma}$? I agree that we can do it without $X$. The term ``generalized exponential'' is only used once in this paper and readers might get confused.}
...
This case corresponds to the classic SIR model, where the infectious period is exponentially distributed \cite{anderson1992infectious}\dc{do we need that many cites?!}\jd{Sort of, but they're already in the table, so I removed most of them here.}
...
\dc{I like this new paragraph below. It's very clear and a strong part of the paper.}
\jd{Your kind words are much appreciated.}
