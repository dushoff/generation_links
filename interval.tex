...
\dc{Which reference? I don't recall wanting to cite myself... probably a glitch with references? Ignore?}
\jsw{If not on arxiv, don't cite.}
...
\hotcomment\jd{Get Hope-Simpson (or else the summary from Bailey, in which case cite also Bailey, and fix citations above, then I think we can ship.)}
...
(dashed curve) The Gamma approximated speed-strength relationship \eref{gamApp}, using the mean and CV of a pseudo-realistic generation-interval distribution. 
		...
		Dashed curves represent estimated distribution of generation intervals using method of moments and MLE (corresponding to approximate speed-strength relationships of the left figure). \swp{check this figure caption as well}
	}
...
\jsw{Losing me... so it is not good in the specific case, but it would actually be worse in the general case? So we should be satisfied or unsatisfied? Or is the point that w/ rabies any kind of inference is hard because inferring R when near 1 is generally a hard problem?}
\jd{Better now?}
...
\jsw{Somehow the tense gets weird here talking about the past in the present} \swp{slightly modified}
...
\dc{OK, up to you. You have change figure S3 in my direction, so I'm happy enough;)}
...
The authors thank Steve Bellan for helpful comments. J.S.W. acknowledges support from Army Research Office grant W911NF-14-1-0402.
