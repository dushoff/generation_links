...
\swp{Starting with basic reproduction number? Reviewer 2 wants us to use Ro instead of R. I think we have to make a distinction.}
Infectious disease research often focuses on estimating the basic reproduction number \Ro, i.e., the number of new infections caused on average by a single primary infection in a fully susceptible population.
The basic reproduction number provides information about the disease's potential for spread and the difficulty of control.
It is described in terms of an average \cite{AndeMay91} or an appropriate sort of weighted average \cite{DiekHees90}.

The basic reproduction number has remained a focal point for research because it provides information about how an infection spreads in a population, on the scale of disease generations.
...
Estimates of the initial exponential rate of spread, $r$, can then be combined with a mechanistic model that includes unobserved features of the infection to esimate the \swp{effective} reproduction number \RR, i.e., the number of new infections caused on average by a single infection.
...
\revcomment{This is nonsense. There are whole books about the gamma function and its properties are taught to undergraduates.}
\swp{Changed the last sentence to clarify}
...
\end{figure}
\revcomment{What is the justification for including $\rho > 1$?}
