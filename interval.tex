...
Infectious disease research often focuses on estimating the reproduction number, i.e., the number of new infections caused on average by a single infection.
This number is termed the reproduction number -- \RR.
The reproduction number provides information about the disease's potential for spread and the difficulty of control.
...
Estimates of the initial exponential rate of spread, $r$, can then be combined with a mechanistic model that includes unobserved features of the infection to esimate the initial reproduction number, \RR.
...
In this section, we introduce an analytical framewok, and recapitulate previous work relating growth rate $r$ and reproduction number \RR.
...
\revcomment{What does this mean? The normal only takes positive values too.}
\swp{Not sure what the reviewer means here...}
