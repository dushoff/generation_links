...
\jd{K. Just try to do IMRD an dkeep this in jmind.}

Given an empirical distribution of generation intervals, we obtain the theoretical relationship between exponential growth rate and reproductive number, we can discretize \eref{Euler} and subtitute the empirical distribution:
\begin{equation}
...
We obtain the gamma approximation by estimating moments from data and using \eref{genExp}; these moments can be calculated directly, or estimated using a maximum-likelihood fit.
...
\jd{Yes. I was not saying anything about this P., just the one before it.}
