...
we discuss them separately as they lead to interesting theoretical properties and motivate framework for \ref{sub:seminr}.
...
Then, latent period follows a gamma distribution with mean $1/\sigma$ and square coefficient of variation $1/m$;
...
In particular, \cite{roberts2007model} proved that $\RR$ is an increasing function of $m$ and $n$ given that exponential growth rate, mean generation interval and mean exposed period are fixed.
...
In fact, the second order taylor expansion of \rR\ relationship (\eref{taylor}) can be generalized for any generation interval distributions:
\begin{equation}
...
\swp{I moved this here for now because I didn't know where else to put... If we decide to remove the previous section, we should put this back}
