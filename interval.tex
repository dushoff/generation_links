...
\cite{WallLips07} looked at \rR\ relationships under various distributional assumptions (e.g., exponential, delta, and normal) and explored how their underlying parameters affect the \rR\ relationsip.
On the other hand, \cite{wearing2005appropriate, roberts2007model} mainly focused on \rR\ relationship derived from compartmental models and studied consequences of changes in incubation and infectious period distributions.
However, \cite{wearing2005appropriate, roberts2007model} presented seemingly contradictory results that more variable infectious period distribution leads to opposite effect in \RR.
...
\swp{No. I think there's something about constant contact rate. If we relax that assumption, we might see a difference. For example, if infections are concentrated during early phase (due to control measures such as hospitalization or characteristic of a disease such as HIV), it matters less when they recover and effect on mean might be small. On the other hand, if infections are concentrated near the end of infectious periods, effect on mean will be large.}
