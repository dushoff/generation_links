...
We then calculate ``exact'' relationships (for our realistic distributions) by substituting sampled generation intervals into the exact speed-strength relationship \eref{Euler}.
...
(solid curve) The exact speed-strength relationship \eref{Euler} using a realistic generation-interval distribution based on \cite{AylwBarb14}.
...
\begin{figure}[htbp] \centering
	...
	\caption{Estimating \RR~for measles.
		(solid curve) The exact speed-strength relationship \eref{Euler} using a realistic generation-interval distribution.
		(dashed curve) The gamma-approximated relationship \eref{gamApp} using the mean and CV of a realistic generation-interval distribution (this curve is almost invisible because it overlaps the solid curve)
		(dotted curves) Naive approximations based on exponential (lower) and fixed (upper) generation distributions.
...
(dashed curve) The Gamma approximated speed-strength relationship \eref{gamApp}, using the mean and CV of a realistic generation-interval distribution. 
		...
		(dotted curves) Naive approximations based on exponential (lower) and fixed (upper) generation distributions.
		Points indicate estimates for the two countries calculated by \cite{HampDusho09}: {Ngorongoro (square, $\RR=1.14$), and Serengeti (triangle, $\RR=1.19$).}
...
\hline
Relative length of generation interval ($\rho$) & 0.35, 0.45, 0.68 \cite{AylwBarb14} & NA & 0.15, 0.17 \cite{HampDush09} \\
\hline
