...
\swp{
A few studies have explored how changes in certain features of the generation interval distributions translates to changes in the \rR\ relationship \cite{wearing2005appropriate, WallLips07, roberts2007model}.
Their results, although useful, rely mostly on mathematical arguments and assume different conditions.
What is missing is a general interpretation that can unify these results and a clear biological intution.
}
...
\swp{This case is also equivalent to the generation interval distribution of an SIR model where infectious period is exponentially distributed \cite{anderson1992infectious, ferguson2005strategies, pybus2001epidemic, WallLips07}}

\swp{
Characterizing the \rR\ relationship with mean and coefficient of variation provides a simple explanation for the results derived from epidemiological models \cite{wearing2005appropriate, roberts2007model}.
As mean and variance of latent and infectious periods are directly linked to mean and variance of generation intervals, one can easily translate the effect of changes in moments of latent and infectious periods to changes in \RR:
shorter mean latent and infectious periods are expected to result in smaller \RR\ and tighter latent periods are expcted to result in higher \RR.
However, as decreasing coefficient of variation of infectious period leads to shorter mean generation interval as well \cite{Sven07}, assuming a tighter infectious period distribution does not necessarily result in higher \RR \cite{wearing2005appropriate}.
}

\swp{
This phenomenon explains seemingly inconsistent results by \cite{wearing2005appropriate, roberts2007model} that predicted opposite effects of decreasing coefficient of variation of infectious periods on \RR\ under the same epidemiological models.
Decreasing coefficient of variation of infectious period results in higher \RR, assuming that mean generation interval is fixed \cite{roberts2007} (hence only affecting variation in generation intervals).
On the other hand, decreasing coefficient of variation of infectious period, while keeping infectious period fixed, decreases both mean and variation in generation intervals and lowers \RR\ \cite{wearing2005appropriate}.
As effect of decrease in mean generation interval (which decreases \RR) is stronger than decrease in variation in generation intervals (which increases in \RR), 
we observe an overall decrease in \RR.
}
...
More realistic methods, such as empirical and SEIR methods, may yield more realistic estimates but sufficient amount of data may not be available early in an epidemic.
On the other hand, simple methods, such as delta and exponential approximations, require minimal information (i.e., mean generation interval) but result in extreme estimates. 
}
...
\swp{I also edited this paragraph:}
