...
Infectious disease research often focuses on estimating the reproduction number \RR, i.e., the number of new infections caused on average by a single infection, and the related basic reproduction number \Ro, the value of \RR\ in for a single primary infection in a fully susceptible population.
...
Approaches based on $r$, including the approach taken in this paper, implicitly assume that \RR\ (and the generation distribution) remain roughly constant as an outbreak takes off, for at least long enough that it makes sense to estimate a rate of exponential growth. This is indeed what would be expected from simple models, but there are concerns about how long this exponential period is, how best to fit the ``initial'' rate of growth \cite{MaJDush14} and whether outbreak data might better be fitted using ``sub-exponential'' models \cite{Chowell}. TODO
...
\swp{Reviewer 1 claims that Roberts predates Nishiura. This is not true. We can explain this to the reviewer.}
