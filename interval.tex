...
\swp{In contrast to gamma distributed incubation and infectious periods assumed by \cite{AylwBarb14}}, we used a lognormal assumption for our components because it is straightforward and should provide a challenging test of our gamma approximation \swp{(see Appendix for results using gamma components)}. 
...
Infectious periods were assumed to follow a gamma distribution with coefficient of variation of 0.2 as it is a more realistic distribution than exponential distribution that have often been used in previous measles models \cite{Lloy01}.
Given that there is small variation in infectious periods \cite{keeling1997disease}, this is a reasonable assumption.
}
...
We did a similar analysis for rabies by constructing a pseudo-realistic from observed incubation and infectious period distributions (see \fref{rabiefsCurve}).
