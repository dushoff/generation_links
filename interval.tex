...
\dc{I have already said it in comments on previous drafts: this is misrepresenting the work of Wallinga Lipsitch 2007. They give the example of a normal distribution for $g$ among three other examples (and the normal example is the shortest). If anything, they emphasized their work on empirical distributions. Hence, I don't think it's fair to say it's their approach. We should also find other publications that used the normal approximation (if any). }
...
The limit as $\kappa\to 0$ is reasonably easy to interpret. The incidence is increasing by a factor of $\exp(\rho)$ in the time it takes for an average disease generation. If $\kappa=0$, the generation interval is fixed, so the means\dc{fix typo} the average case must cause $\RR = \exp(\rho)$ new cases.
...
We test our approximation method by generating\dc{``simulating''?} pseudo-realistic generation-interval distributions, using observations (or estimated parameters) of generation intervals (or else latent and infectious intervals) from specific diseases. 
...
\dc{The description that follows is really not clear(``To generate generation intervals...''). Please write down with a mathematical formulation, it shouldn't be too long and will have the benefit of being unambiguous... }
...
\dc{for all these examples, do you pay attention to the fact that the observed GIs are \emph{backward} and not intrinsic? Is any adjustment made? If so, we should at least have a short paragraph about it.}
...
Here, we found surprisingly close agreement between the exact and approximate relationships between $r$ and \RR\ across a much wider range of interest (a difference of $<1\%$ for \RR\ up to $>20$). \dc{Reference to Figure 4}
...
The gamma approximation for calculating \RR\ from $r$ was introduced by \cite{NishCast09}, and  provides estimates that are simpler, more robust and more realistic than those from the normal approximations developed by \cite{WallLips07} (see Appendix). \dc{Again, it's not fair to associate the normal approximation with Wallinga Lipsitch} 
