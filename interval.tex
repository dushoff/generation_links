...
Points indicate estimates for the three focal countries of the West African Ebola Outbreak calculated by \cite{AylwBarb14}: \swp{Sierra Leone (square, $\RR=1.38$), Liberia (triangle, $\RR=1.51$), and Guinea (circle, $\RR=1.81$).}
...
% We estimated the squared CV $\kappa$ using a maximum-likelikhood fit to a gamma distribution (it is also possible to estimate the squared CV directly, but our way is better, see Appendix) and compared the simulated distribution to a single-gamma approximation.
...
\begin{figure}[htbp] \centering
	...
	\caption{Estimating \RR~from rabies infectious case data.
		...
		\swp{Serengeti (circle, $\RR=1.06$) and Ngorongoro (triangle, $\RR=1.32$).}
	}
...
\swp{Move the figure to an appendix, along with the s.\ that explains how we generate it.}
\begin{figure}[htbp] \centering
...
\jsw{For this paper, I really think 5 must precede 4. You have an approximation method in 3. But then you apply this to data... Then you tell us what you actually did. No good! I want to know about the discretization before I judge your fits. Please move this up to 4, or break section 3 into two parts:  3.1 Approximation method, theory; 3.2 Approximation method, in practice.}
...
\swp{Now, I agree with JSW that this should go before example. I like the idea of having two sections called (1) approximation method, in theory and (2) approximation method, in practice. Adding a paragraph like this might be helpful:}

Given an empirical distribution of generation intervals, we can simply use the observed moments of the distribution to apply the gamma approximation. To obtain the theoretical relationship between exponential growth rate and reproductive number, we can discretize the equation \ref{eq:Euler} and subtitute the empirical distribution:
\begin{equation}
1/\RR = \frac{1}{N} \sum_{i=1}^{N} \exp(-x_i/C),
\label{eq:obsR}
\end{equation}
where $x_i$ represent a sample and $N$ is the total number of samples. If we interpret $\int g(\tau) \exp(-\tau/C) d\tau$ as a weighted mean of $\exp(-\tau)/C$, equation \ref{eq:obsR} is an intuitive transition from a continuous distribution to a discrete data.

\swp{we need to say something about the histogram based method by WL here... This is not sensitive to the histogram widths and is more intuitive IMHO.}

In other cases, we may be given empirical distributions of incubation and symptomatic periods (or their estimated distributions and respective parameters) instead. For simplicity, we can assume that latent and infectious period follow the same distributions as incubation and symptomatic periods respectively (see [CITE] for further dicussion). \swp{I don't know how much we want to talk about this here.}

\swp{Talk about how we can estimate the moments of the generation interval distribution from latent/infectious periods and even the full distribution. Making a waiting time analogy might help the readers.}

\swp{If we decide to incorporate what I wrote above, we can clean up the following paragraphs and it might be easier to format the paper:}
...
We then re-sampled the infectious periods from our samples, weighting each sample by infectious-period length (since individuals with longer infectious periods have more time to transmit). 
Generation intervals were drawn (uniformly) at random from the resulting infectious period (i.e., between the end of the latent period and the end of the infectious period).

\jd{JD needs to figure out why he did something weird about Ebola and whether he agrees with SWP about not doing it.}

\swp{Since we're using the lognormal anyway, the way we're trying to match WHO doesn't quite work.}
...
% To investigate the effect of small samples, we used our full samples to draw the reference curves in figures \frange{EbolaCurve}{rabiesCurve}, and subsampled from these for the sampling example \fref{ebolaSample}. We used \eref{obsR} to draw curves related $r$ to $\RR$.
