...
This is a better-grounded starting point than the normal-distribution approach mentioned by \cite{WallLips07}, since the gamma distribution is confined to non-negative values (see Appendix for comparisons).
...
The limit as $\kappa\to 0$ is reasonably easy to interpret. The incidence is increasing by a factor of $\exp(\rho)$ in the time it takes for an average disease generation. If $\kappa=0$, the generation interval is fixed, so the the average case must cause exactly $\RR = \exp(\rho)$ new cases.
...
We test our approximation method by generating\dc{``simulating''?} \jd{Weitz objected to our use (overuse?) of simulating in this context} pseudo-realistic generation-interval distributions, using observations (or estimated parameters) of generation intervals (or else latent and infectious intervals) from specific diseases. 
