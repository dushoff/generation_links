...
Generation interval distributions can be difficult to ascertain empirically \dc{Cite Champredon,Dushoff RSPB 2015: where we show the difficulty of such estimation}, and the generation-function approach depends on an entire distribution -- which makes it difficult to determine which features of the distributions are essential to connect measurements of the rate of spread $r$, with the reproductive number, \RR.
...
Disease growth is predicted \dc{``assumed'' rather than ``predicted''? See Chowell's work on how often sub-exponential growth occurs in real-life epidemics} to be approximately exponential in the early phase of an epidemic, because the depletion in the effective number of susceptibles is relatively small.
...
The longer the generation interval is compared to $T_c$ \dc{you mean $\Tc$, right?}, the higher the estimate of $\RR$ (see \fref{link}).
...
This case corresponds to the classic SIR model, where the infectious period is exponentially distributed \cite{anderson1992infectious, ferguson2005strategies, pybus2001epidemic, WallLips07}\dc{do we need that many cites?!}
...
These approximations are first done with large amounts of data, allowing us to evaluate how well the approximations describe the \rR\ relationship under ideal conditions, and then tested with smaller amounts of data \dc{explain why: to assess how our method can perform in a more realistic framework, where data availability if often challenging}. 
