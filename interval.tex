...
The reproduction number has remained a focal point for research because it provides information about how an infection spreads in a population, on the scale of disease generations.
...
Estimates of the initial exponential rate of spread, $r$, can then be combined with a mechanistic model that includes unobserved features of the infection to esimate the effective reproduction number \RR, i.e., the number of new infections caused on average by a single infection.
...
\hline
Exponential approximation (SIR) & \tabitem Mean generation interval & \tabitem Assumes exponentially distributed generation interval & \cite{AndeMay91, pybus2001epidemic, ferguson2005strategies, wallinga2007generation, trichereau2012estimation} \\
...
\hline
SEIR (gamma) & \tabitem Mean/SD latent period & \tabitem Assumes gamma distributed latent and & \cite{anderson1980spread, wearing2005appropriate, wallinga2007generation, roberts2007model} \\
...
\swp{I suggest that we move this subexponential to discussion... I don't think it fits in the intro}
...
Finally, restricting the domain to only non-negative values makes it more biologically realistic than the normal, although other approximations also have this property.
...
\revcomment{Make this paragraph clearer:}
\swp{JD: I tried.}
...
when mean infectious period is held constant, less-variable infectious periods reduces the overall length of generation intervals, leading to higher \RR\ \cite{wearing2005appropriate}.
...
\swp{I moved subexponential here and changed things a bit}

The approaches based on $r$, including the approach taken in this paper, implicitly assume that \RR\ (and the generation distribution) remain roughly constant as an outbreak takes off.
The initial exponential growth assumption is supported by simple models \cite{AndeMay91}, but there are concerns about how long this exponential period is and whether outbreak might better be characterized by ``sub-exponential'' models \cite{viboud2016generalized, chowell2016characterizing}.
For example, appropriate time frame in which the exponential assumption holds may be as short as 2-4 generations for EVD \cite{chowell2015western}.
Deceleration in the initial growth rate can be driven by many factors (e.g., heterogeneity in population structure, behaviour change, and early intervention), and failing to account for such patterns can result in overestimation of reproduction number \cite{viboud2016generalized}.
However, we still expect qualitative roles of generation-interval distribution in shaping the \rR\ relationship to be similar regardless of initial growth patterns of an epidemic (e.g., see Figure 2 from \cite{chowell2016characterizing}).
...
\revcomment{Bibliogrphy style is inconsistent}
